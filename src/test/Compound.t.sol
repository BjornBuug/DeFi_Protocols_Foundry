...
contract ContractTest is Test {
  ...
  function testSupplyRedeem() public {
    ...
    // Note: supplyRatePerBlock to dynamically adjust the interest rate based on supply and demand within the same block.
    // Note: Returns the current per-block supply interest rate for this cToken
    // Note: This repesent the interest rate for very short period in single block.
    supplyRate = C_WBTC.supplyRatePerBlock(); //
    emit log_named_decimal_uint("supplyRate:", supplyRate, 18);
    ...
    C_WBTC.mint(100000000); // supply 1 btc and get c_WBTC

    console.log("----After supplying, all status:----");
    emit log_named_decimal_uint("Supplied 1 btc to get C_WBTC:", C_WBTC.balanceOf(address(this)), 8);
    ...
    estimateBalance = estimateBalanceOfUnderlying();
    emit log_named_decimal_uint("estimateBalance:", estimateBalance, 18);

    // calculates and returns how much it's worth the ctokens held by the in underlying asset(WBTC)
    balanceOfUnderlying = C_WBTC.balanceOfUnderlying(address(this)); 
    emit log_named_decimal_uint("balanceOfUnderlying:", balanceOfUnderlying, 8);
  ...
  function estimateBalanceOfUnderlying() public returns (uint) {
    ...
    uint exchangeRate = C_WBTC.exchangeRateCurrent();  // 18 decimals
    
    uint decimals = 8; // WBTC = 8 decimals
    uint cTokenDecimals = 8; // 8 decimals
